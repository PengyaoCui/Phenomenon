...
void Integral_OPiRatio(){
//=============================================================================
  //Double_t dNdEta[nc-1]; 
  //CentTodNdEta(0, 1, dNdEta);//CR
  //for(Int_t i = 1; i<nc; i++)cout<<dCent[i-1]<<"-"<<dCent[i]<<" = "<<dNdEta[i-1]<<endl;
  
  auto hD(GetDataC("data/HEPData.root", 39)); 
  auto gD = GetDataE("data/HEPData.root", 39); 
  TGraph* g[3];
  g[0] = RatioToPi(1, 0, 3);    //Para1: "pp13TeV", "pp7TeV" 
  g[1] = RatioToPi(1, 1, 3);    //Para2: "SoftQCD_CR", "SoftQCD_Rope", "SoftQCD_CRandRope"
  g[2] = RatioToPi(1, 2, 3);    //Para3:"Kshort", "Lambda", "Xi", "Omega", "Phi", "Pion", "Kion", "Proton"

//=============================================================================
  auto dflx(0.), dfux(30.);
  auto dfly(0.), dfuy(0.001);
