...
void f16_Incl_KstarK_Cent_Rope(){
//=============================================================================

  TH1D* h1[3]; TH1D*h2[3]; TH1D* hR[3]; TGraph*g[3];//Double_t dndeta[3];
  Int_t c[] = {0, 10, 40, 100}; Int_t n = sizeof(c)/sizeof(Int_t) - 1;
  for(int i=0; i<n; i++)h1[i] = PtSpectrum(1, 1, 0, c[i], c[i+1], kFALSE, kFALSE); //Para1: "pp13TeV", "pp7TeV" 
  for(int i=0; i<n; i++)h2[i] = PtSpectrum(1, 1, 8, c[i], c[i+1], kFALSE, kFALSE); //Para2: "SoftQCD_CR", "SoftQCD_Rope", "SoftQCD_CRandRope"
...
//=============================================================================
  ...
  auto can(MakeCanvas("KstarKRatio_Incl_Cent_rope"));
  //can->SetLogy();
