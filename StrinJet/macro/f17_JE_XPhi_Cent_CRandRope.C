...
void f17_JE_XPhi_Cent_CRandRope(){
//=============================================================================

  TH1D* h1[3]; TH1D*h2[3]; TH1D* hR[3]; TGraph*g[3];//Double_t dndeta[3];
  Int_t c[] = {0, 10, 40, 100}; Int_t n = sizeof(c)/sizeof(Int_t) - 1;
  for(int i=0; i<n; i++)h1[i] = PtSpectrum(1, 2, 4, c[i], c[i+1], kTRUE, kFALSE); //Para1: "pp13TeV", "pp7TeV" 
...
//=============================================================================
  ...
  auto can(MakeCanvas("XPhiRatio_JE_Cent_cr_rope"));
  //can->SetLogy();
