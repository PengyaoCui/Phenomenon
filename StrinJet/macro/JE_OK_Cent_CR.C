...
//=============================================================================
  ...
  for(int i=0; i<n; i++)h1[i]  = PtSpectrum(1, 0, 0, c[i], c[i+1], kTRUE, kFALSE); //Para1: "pp13TeV", "pp7TeV" 
  for(int i=0; i<n; i++)h2[i]  = PtSpectrum(1, 0, 3, c[i], c[i+1], kTRUE, kFALSE); //Para2: "SoftQCD_CR", "SoftQCD_Rope", "SoftQCD_CRandRope"
  for(int i=0; i<n; i++)h1u[i] = PtSpectrum(1, 0, 0, c[i], c[i+1], kFALSE, kTRUE); //Para3:"Kshort", "Lambda", "Xi", "Omega", "Phi", "Pion", "Kion", "Proton"
...
//=============================================================================
  ...
  auto can(MakeCanvas("OKRatio_JE_Cent_cr"));
  //can->SetLogy();
