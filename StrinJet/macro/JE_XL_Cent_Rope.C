...
void JE_XL_Cent_Rope(){
//=============================================================================

  TH1D* h1[3]; TH1D*h2[3]; TH1D* h1u[3]; TH1D*h2u[3]; TH1D* hR[3]; //Double_t dndeta[3];
  Int_t c[] = {0, 10, 40, 100}; Int_t n = sizeof(c)/sizeof(Int_t) - 1;
  for(int i=0; i<n; i++)h1[i]  = PtSpectrum(1, 1, 1, c[i], c[i+1], kTRUE, kFALSE); //Para1: "pp13TeV", "pp7TeV" 
  for(int i=0; i<n; i++)h2[i]  = PtSpectrum(1, 1, 2, c[i], c[i+1], kTRUE, kFALSE); //Para2: "SoftQCD_CR", "SoftQCD_Rope", "SoftQCD_CRandRope"
  for(int i=0; i<n; i++)h1u[i] = PtSpectrum(1, 1, 1, c[i], c[i+1], kFALSE, kTRUE); //Para3:"Kshort", "Lambda", "Xi", "Omega", "Phi", "Pion", "Kion", "Proton"
  for(int i=0; i<n; i++)h2u[i] = PtSpectrum(1, 1, 2, c[i], c[i+1], kFALSE, kTRUE); //Para4:Centmin 
                                                                                  //Para5:Centmax 
...
//=============================================================================
  ...
  TString stny("#Xi/#Lambda");
  
  SetStyle(kTRUE);
  gStyle->SetErrorX(0);
  
  auto can(MakeCanvas("XLRatio_JE_Cent_rope"));
  //can->SetLogy();
