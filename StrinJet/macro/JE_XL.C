...
//=============================================================================
  ...
  for(int i=0; i<3; i++)h1[i] = PtSpectrum(1, i, 1, kTRUE, kFALSE); //Para1: "pp13TeV", "pp7TeV" 
  for(int i=0; i<3; i++)h2[i] = PtSpectrum(1, i, 2, kTRUE, kFALSE); //Para2: "SoftQCD_CR", "SoftQCD_Rope", "SoftQCD_CRandRope"
  for(int i=0; i<3; i++)h1u[i] = PtSpectrum(1, i, 1, kFALSE, kTRUE);//Para3:"Kshort", "Lambda", "Xi", "Omega", "Phi", "Pion", "Kion", "Proton"
  for(int i=0; i<3; i++)h2u[i] = PtSpectrum(1, i, 2, kFALSE, kTRUE);//Para4: jet cone: kTRUE, kFALSE
...
//=============================================================================
  ...
  auto can(MakeCanvas("XLRatio_JE"));
  //can->SetLogy();
