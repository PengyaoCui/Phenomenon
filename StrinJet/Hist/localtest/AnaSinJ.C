...
//--Get NFwd track distribution------------------------------------------------
  ...
  for(int p = 0; p<1; p++){
    ...
    auto hJetFwd((TH1D*)lJ->FindObject(Form("hJetFwd")));
    TH1D* hJet = new TH1D(Form("hJet%s", sp[p].Data()), "", 400, 0., 40);
    for(Int_t i = 1; i< nc; i++){
...
//==Ratio to Kshort Vs Particle to Jet distance================================
  ...
  Double_t dA[] = {0.0037, 0.015-0.0037, 0.033-0.015, 0.06-0.033};
  ...
  hPJK->Scale(1./(dJEvent*2.*0.75*TMath::TwoPi()));
  for(int i = 1; i<= hPJK->GetNbinsX(); i++){
    ...
    dvK = (dvK/dA[i-1] - dKUE); deK = deK/dA[i-1];
  ...
  for(int p = 0; p<np-3; p++){
    ...
    hPJP->Scale(1./(dJEvent*2.*0.75*TMath::TwoPi()));
    ...
    for(int i = 1; i<= hPJP->GetNbinsX(); i++){
      ...
      dvP = (dvP/dA[i-1] - dUE); deP = deP/dA[i-1]; 
