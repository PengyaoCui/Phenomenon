...
//=============================================================================
  ...
  for(int i = 0; i < 6; i++ ){
    ...
    if(i!=0)DrawGraph(g[0][i], wcl[i], "L");
    if(i!=0)DrawGraph(g[1][i], wcl[i], "L");
    if(i!=0)DrawGraph(g[2][i], wcl[i], "L");
  }
  ...
  tex->DrawLatex(0.16, 0.92, "pp #sqrt{#it{s}} = 7 TeV  Strange particle to K^{0}_{S} ratio in jets");
